//
//  TaskDetailsTableViewController.m
//  TaskManager
//
...
@interface TaskDetailsTableViewController ()

@end

@implementation TaskDetailsTableViewController
...
- (void)viewDidLoad {
    ...
    self.datePicker.date = self.item.finalDate;
    self.priorityLabel.text = [self.item.priority stringValue];
    self.categoryLabel.text = self.item.category;
    self.prioritySlider.value = [self.item.priority floatValue];
    self.backgroundColorCell.backgroundColor = self.item.color;
    self.colorIndex = self.item.colorIndex;
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    ...
    if (sender == self.saveButton && self.titleTextField.text.length > 0 && self.categoryLabel.text.length > 0) {
        self.item.title = self.titleTextField.text;
        self.item.category = self.categoryLabel.text;
        self.item.priority = [NSNumber numberWithFloat:self.prioritySlider.value];
        self.item.finalDate = self.datePicker.date;
        self.item.color = self.backgroundColorCell.backgroundColor;
        self.item.colorIndex = self.colorIndex;
    }
